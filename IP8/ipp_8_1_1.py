...
class drive_sim_t:
  ...
  def __init__(self):
    ...
    # Wir führen für x 2 Variablen ein:
    # x_i:  x zum i.ten Zeitschritt (dem aktuellen)
    # x_i1: x zum (i+1).ten Zeitschritt (dem folgenden)
    # # (y und psi entsprechend)
    """ #ALT
    self.x_i = self.x_i1 = 0.0
    self.y_i = self.y_i1 = 0.0
    self.psi_i = self.psi_i1 = 0.0
  ...
  def run(self,v,Lambda):
    ...
    """ #ALT
    self.x[self.i+1] = self.x_i1      
    self.y[self.i+1] = self.y_i1      
    self.psi[self.i+1] = self.psi_i1  
    """ #ALT
    #""" #NEU
    ...
    # Am Ende der Schleife werden die "neuen" Werte schon mal zu den "alten" Werten für den nächsten Durchlauf  
    """ #ALT
    self.x_i = self.x_i1     
    self.y_i = self.y_i1     
    self.psi_i = self.psi_i1 
